using PSFModels
using PSFModels: free_params, model_from_vector, _has_hessian, _has_deriv, fit, fit_lm, TukeyLoss, weight, FixedScale, LMProblem, lm_irls
...
@testset "struct fit CircularGaussianPSF" begin
    ...
    best, result = fit(init, img, inds; x_abstol = 1.0e-6)
    ...
    best2, result2, cov = fit(init, img, inds; inv_var, x_abstol = 1.0e-6)
    ...
    best3, _ = fit(init_fixed, img, inds; fixed = (bkg = 5.0,), x_abstol = 1.0e-6)
    @test best3.bkg ≈ 5.0
    @test best3.x ≈ truth.x rtol = 1.0e-3

    # LogCoshLoss (still twice differentiable → Newton path)
    best4, _ = fit(init, img, inds; loss = LossFunctions.LogCoshLoss(), x_abstol = 1.0e-6)
    @test best4.x ≈ truth.x rtol = 1.0e-3

    # HuberLoss (not twice differentiable → LBFGS path)
    best5, _ = fit(init, img, inds; loss = LossFunctions.HuberLoss(1.0), x_abstol = 1.0e-6)
    @test best5.x ≈ truth.x rtol = 1.0e-3
end
...
@testset "struct fit GaussianPSF" begin
    ...
    best2, _ = fit(init_fixed, img, inds; fixed = (theta = 15.0,), x_abstol = 1.0e-6)
    @test best2.theta ≈ 15.0
    @test best2.x ≈ truth.x rtol = 1.0e-3

    # With inv_var
    inv_var = fill(1.0, size(img))
    best3, result3, cov = fit(init, img, inds; inv_var, x_abstol = 1.0e-6)
    @test best3.x ≈ truth.x rtol = 1.0e-3
    @test size(cov) == (7, 7)
end
