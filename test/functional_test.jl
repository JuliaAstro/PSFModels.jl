using PSFModels: gaussian, airydisk, moffat, normal, PSFModels
...
function test_model_interface(K)
    ...
    return @test m(0, 0) ≈ val_dir ≈ BigFloat(1)
end
...
@testset "airydisk" begin
    ...
    radius = 1.22 * ld
    # first radius is 0
    @test m(radius, 0) ≈ 0 atol = 1.0e-5
    @test m(-radius, 0) ≈ 0 atol = 1.0e-5
    @test m(0, radius) ≈ 0 atol = 1.0e-5
    @test m(0, -radius) ≈ 0 atol = 1.0e-5

    # second radius is 0
    radius2 = 2.23 * ld
    @test m(radius2, 0) ≈ 0 atol = 1.0e-5
    @test m(-radius2, 0) ≈ 0 atol = 1.0e-5
    @test m(0, radius2) ≈ 0 atol = 1.0e-5
    @test m(0, -radius2) ≈ 0 atol = 1.0e-5
    ...
    r2 = fwhm[2] * 1.18677
    # first radius is 0
    @test m(r1, 0) ≈ 0 atol = 1.0e-5
    @test m(-r1, 0) ≈ 0 atol = 1.0e-5
    @test m(0, r2) ≈ 0 atol = 1.0e-5
    @test m(0, -r2) ≈ 0 atol = 1.0e-5
